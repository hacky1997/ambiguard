...
def paired_auc_delta(a: np.ndarray, b: np.ndarray, labels: np.ndarray,
    ...
    Comparing two independent CIs is the wrong test — it is far too conservative
    and, done the other way round, far too permissive. The paired delta is the
    correct comparison for two statistics scored on the same rows.
    """
    def _auc(s: np.ndarray, y: np.ndarray) -> float:
        pos, neg = s[y == 1], s[y == 0]
        if len(pos) == 0 or len(neg) == 0:
